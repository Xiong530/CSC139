...
int main(int argc, char* argv[])
{
	...
	// Write code to check the validity of the command-line arguments
        if (bufSize < 2 || bufSize > 450){
                printf("The buffer size is not within the valid command-line arguments. Exiting process.\n");
                exit(1);
        }
...
void InitShm(int bufSize, int itemCnt)
{
    int in = 0;
    int out = 0;
    const char *name = "OS_HW1_andrewxiong"; // Name of shared memory object to be passed to shm_open
     ...
     shm_block = shm_open(name, O_CREAT | O_RDWR, 0666);
     ftruncate(shm_block, SHM_SIZE);
     gShmPtr = mmap(0, SHM_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, shm_block, 0);
